...
const Blogs = () => {
  ...
  return (
   ...
   {!loading &&  <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 w-[80vw] mx-auto mt-6 gap-10">
    {blogs.map(e=>{
       return <Link href={`/blogpost/${e.slug}`} className='cursor-pointer'>
      <div className='rounded-lg overflow-hidden p-4 transition-all'>
        ...
        <h1 className='mt-3 text-lg font-bold'>{e.title}</h1> 
          ...
          <div className='text-center'><Link href={`/blogpost/${e.slug}`}><Button variant="outline">Read More</Button></Link></div>
      </div>
      </Link>
    })}
